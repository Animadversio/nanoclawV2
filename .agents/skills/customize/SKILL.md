...
3. **Plan the changes** — Identify the v2 surface the change belongs to (entity model in the central DB, per-agent-group container config, per-group `AGENTS.md`, or core code).
...
| `groups/<folder>/AGENTS.md` | Per-agent-group memory/persona and instructions |
...
Implementation:
- Persona, instructions, and personality live per agent group in `groups/<folder>/AGENTS.md` — edit that file for the target group.
...
Implementation:
- The agent interprets requests naturally — add instructions to the agent group's `groups/<folder>/AGENTS.md`.
