...
Each step is safe to re-run: directory creation uses `mkdir -p`, initial wiki files are created only if absent, the container skill is preserved unless the user opts to update it, and the group AGENTS.md section is replaced in place via marker comments rather than duplicated.
...
Edit the group's AGENTS.md to add a wiki section, wrapped in marker comments so it can be located and replaced on re-run:
