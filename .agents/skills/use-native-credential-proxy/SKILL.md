...
This skill adds a **native, `.env`-based credential path** for the container agent — an explicit opt-out of the OneCLI gateway. With it enabled, NanoClaw reads the Anthropic credential straight from `.env` and threads it into the container as standard environment variables, which the Codex Agent SDK reads natively. No OneCLI vault, no HTTPS proxy, no certificates.
...
```bash
S=.Codex/skills/use-native-credential-proxy
...
```bash
# Native credential proxy (.Codex/skills/use-native-credential-proxy)
...
Ask the user (multiple choice): do they want to use their **Codex subscription** (Pro/Max) or an **Anthropic API key**?

1. **Codex subscription (Pro/Max)** — uses an existing Codex Pro or Max subscription. They run `Codex setup-token` in another terminal to mint a token.
...
**401 errors from the API:** The credential in `.env` is invalid or expired. For a subscription token, re-run `Codex setup-token` and update `CLAUDE_CODE_OAUTH_TOKEN`. For an API key, check it at console.anthropic.com.
