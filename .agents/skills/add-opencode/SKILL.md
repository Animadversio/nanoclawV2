...
**(b)** Add a new standalone `RUN` block for the OpenCode CLI, after the existing per-CLI install blocks (around line 111, right after the `@anthropic-ai/Codex` block). The Dockerfile splits each global CLI into its own layer for cache granularity — keep that pattern; do not collapse them into a single combined `pnpm install -g` call:
...
```bash
cp .Codex/skills/add-opencode/opencode-dockerfile.test.ts src/opencode-dockerfile.test.ts
```
