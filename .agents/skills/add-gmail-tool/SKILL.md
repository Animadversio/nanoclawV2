...
Both integration points this skill relies on live in the container (Bun) tree — the Dockerfile package install and the dynamic allow-pattern derivation in `Codex.ts` — so the guards go there. `cp` overwrites, so re-running is safe.

```bash
S=.Codex/skills/add-gmail-tool
...
- `gmail-allow-pattern.test.ts` asserts `Codex.ts` still spreads `Object.keys(this.mcpServers).map(mcpAllowPattern)` into `allowedTools` — the derivation that makes registering `gmail` (Phase 3) enough to expose `mcp__gmail__*`.
...
Then find the last pnpm global-install `RUN` block (the one that installs `@anthropic-ai/Codex`) and add a new block directly after it (before the `# ---- ncl CLI wrapper` section):
...
The Gmail allow-pattern is derived automatically. `container/agent-runner/src/providers/Codex.ts` builds `allowedTools` from each group's `mcpServers` map (`Object.keys(this.mcpServers).map(mcpAllowPattern)`), so registering `gmail` in Phase 3 exposes `mcp__gmail__*` to the agent.
