...
`container/agent-runner/src/providers/Codex.ts` derives the allow-pattern dynamically from each group's `mcpServers` map (`Object.keys(this.mcpServers).map(mcpAllowPattern)`), so registering `calendar` in Phase 3 automatically allows `mcp__calendar__*`.
...
```bash
cp .Codex/skills/add-gcal-tool/gcal-dockerfile.test.ts src/gcal-dockerfile.test.ts
