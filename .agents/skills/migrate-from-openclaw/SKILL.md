...
- **Container isolation.** Each agent group runs in its own Linux container.
  ...
  AGENTS.md); an OpenClaw chat/group maps to a v2 *messaging group*; the wiring
  row connects them.
- **Shared vs per-group context.** v2 has no `groups/global/`. Shared
  instructions live in `container/AGENTS.md` (mounted read-only into every
  container). Per-group memory is `groups/<folder>/Codex.local.md`; the
  `AGENTS.md` in each group is **composed at spawn — do not edit it.**
...
- **Identity & Memory** — paths of files created, which Codex.local.md edited
...
memory, and Codex.local.md. Shared instructions are placed in
`container/AGENTS.md` (read-only in every container). There is no
`groups/global/`.
...
1. **Shared base (recommended if it was one bot)** — core personality/identity
   goes in `container/AGENTS.md` (every container sees it); each group adds its
   own `Codex.local.md` on top.
...
- **Shared base:** core identity/personality → `container/AGENTS.md` (seen by
  all containers, read-only). Group specifics → that group's
  `groups/<folder>/Codex.local.md`.
- **Fully separate / primary only:** everything → the primary agent's
  `groups/<folder>/Codex.local.md`.

Never edit a group's composed `AGENTS.md` — it's regenerated each spawn. Edit
`Codex.local.md` (or `container/AGENTS.md` for the shared base).
...
- **Always loaded** (core traits, communication style, key rules) → weave into
  `container/AGENTS.md` (shared) or the group's `Codex.local.md` (separate).
- **Reference** (backstory, extended guidelines) → a separate
  `groups/<folder>/soul.md`, with a one-line pointer in `Codex.local.md`:
...
Create `groups/<folder>/user-context.md` and add a pointer in `Codex.local.md`.
Ask whether any critical facts (name, timezone, key prefs) should go directly
into `Codex.local.md` for always-on awareness.
...
1. **Copy as-is** — `cp <workspace>/memory/*.md <group_dir>/daily-memories/`,
   add a pointer in `Codex.local.md`.
...
`Codex setup-token` and register the fresh token.
...
- Identity/personality → `container/AGENTS.md` (shared) or per-group
  `Codex.local.md` + `soul.md`
