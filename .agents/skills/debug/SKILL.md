...
src/container-runner.ts                    container/agent-runner/src/
    ...
    ├── <group>/.Codex-shared ──────────────> /home/node/.Codex
...
**Container identity:** the container runs as user `node` with `HOME=/home/node`. Per-group Codex state (settings, session history) lives in `<group>/.Codex-shared` on the host, mounted to `/home/node/.Codex`.
...
/workspace/agent      ← agent group folder (cwd; AGENTS.md, skills, working files)
/home/node/.Codex    ← per-group .Codex-shared (Codex state, settings, history)
...
The default provider wraps the Codex Agent SDK in `container/agent-runner/src/providers/Codex.ts`. The query is configured roughly as:
