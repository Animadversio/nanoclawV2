...
> 3. `ls .Codex/skills/` — installed skills, then for each `add-*` skill read its `SKILL.md` to learn which files it fetches/writes (e.g. `src/channels/<name>.ts`, `import './<name>.js';` in a barrel, pinned deps)
>
> Report: (a) list of installed `add-*` skills and the files each one owns, (b) list of all changed files, (c) any custom skill directories under `.Codex/skills/` not matching an upstream `add-*` skill.
